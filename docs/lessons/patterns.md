...
- Before writing code to answer a "not delivered / missing" verdict, verify the committed state first
  (`git log` + `git status`): the work may already be committed and the verdict stale, so re-implementing
